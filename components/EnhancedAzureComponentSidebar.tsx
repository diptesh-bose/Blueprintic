...
const AzureComponentSidebar: React.FC<AzureComponentSidebarProps> = ({ onDragStart }) => {
  ...
  const handleGroupDragStart = (event: React.DragEvent, group: { id: string; name: string; color: string; description: string }) => {
    console.log('🚀 GROUP DRAG START - Group:', group.name);
    
    // Set the type for groups
    setType('azureGroup');
    console.log('📝 Type set to: azureGroup');
    
    event.dataTransfer.effectAllowed = 'move';
    
    // Store group data
    const groupDataString = JSON.stringify({
      id: `group-${Date.now()}`,
      name: group.name,
      groupType: group.id,
      color: group.color,
    ...
    event.dataTransfer.setData('application/json', groupDataString);
    event.dataTransfer.setData('application/azure-group', groupDataString);
    
    console.log('📦 Group data:', groupDataString);
  };
  ...
  return (
    <div className="w-80 bg-white border-r border-gray-200 flex flex-col h-full">
      ...
      <div className="border-b border-gray-200 bg-gray-50">
        <div className="p-3">
          <h3 className="text-sm font-semibold text-gray-700 mb-2">Containers & Groups</h3>
          <div className="space-y-1">
            {[
              { id: 'resource-group', name: 'Resource Group', color: '#0078d4', description: 'Organize related resources' },
              { id: 'subnet', name: 'Subnet', color: '#107c10', description: 'Network segmentation' },
              { id: 'tier', name: 'Application Tier', color: '#d83b01', description: 'Logical separation' }
            ].map((group) => (
              <div
                key={group.id}
                draggable
                onDragStart={(e) => handleGroupDragStart(e, group)}
                className="flex items-center gap-2 p-2 bg-white rounded-md border border-gray-200 cursor-grab hover:bg-gray-50 transition-colors drag-item"
              >
                <div
